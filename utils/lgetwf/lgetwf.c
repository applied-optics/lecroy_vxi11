...
double		double_ret;
	...
	while(index<argc){
		...
		if(sc(argv[index],"-bytes_per_point")||sc(argv[index],"-b")||sc(argv[index],"-bytes")){
		...
		if(sc(argv[index],"-segmented")||sc(argv[index],"-seg")||sc(argv[index],"-seq")){
	...
	if(got_file==FALSE||got_scope_channel==FALSE||got_ip==FALSE){
		...
		printf("-ip    -ip_address     -IP      : IP address of scope (eg 128.243.74.78)\n");
		printf("-f     -filename       -file    : filename (without extension)\n");
		printf("-c     -scope_channel  -channel : scope channel (1,2,3,4)\n");
		printf("                                                (A=F1, B=F2, C=F3, D=F4)\n");
		printf("                                :               (S=M1, T=M2, U=M3, V=M4)\n");
		...
		printf("-seg   -segmented      -seq     : set no of segments\n\n");
		printf("OUTPUTS:\n");
	...
	/* Check if we've specifically requested 8-bit transfers, if so, set it up */
		...
		if(got_segmented_averages == TRUE) {
			chnl = lecroy_set_segmented_averages(clink, chnl, no_averages);
			}

		if(got_no_segments == TRUE) lecroy_set_segmented(clink, no_segments);
...
//		printf("Returned value: %g\n",double_ret);
		...
		buf_size = lecroy_write_wfi_file(clink, wfiname, chnl, progname, 1, bytes_per_point, timeout);
		actual_npoints = buf_size / (bytes_per_point * no_segments);
		printf("Bytes per trace (channel %c): %ld; pts/trace: %ld; sample rate: %gSa/S\n", chnl, buf_size, actual_npoints, actual_s_rate);
